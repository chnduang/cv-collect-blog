# JavaScript事件循环-微任务和宏任务

> [https://mp.weixin.qq.com/s/V7V58zBHrIr8WQzTD1DqUg](https://mp.weixin.qq.com/s/V7V58zBHrIr8WQzTD1DqUg)
>
> [https://mp.weixin.qq.com/mp/appmsgalbum?__biz=Mzg5MTU5ODYxOA==&action=getalbum&album_id=1807966784101122055&scene=173&from_msgid=2247485925&from_itemidx=1&count=3&nolastread=1#wechat_redirect](https://mp.weixin.qq.com/mp/appmsgalbum?__biz=Mzg5MTU5ODYxOA==&action=getalbum&album_id=1807966784101122055&scene=173&from_msgid=2247485925&from_itemidx=1&count=3&nolastread=1#wechat_redirect)
...
![图片](https://gitee.com/qdzhou/img-upload/raw/master/images/202202111754466.png)
...
2. 如果一项任务执行花费的时间过长，浏览器将无法执行其他任务，例如处理用户事件。
   ...
   因此，在一定时间后，浏览器会抛出一个如“页面未响应”之类的警报，建议你终止这个任务。
...
我们可以通过将大任务拆分成多个小务来避免这个问题。高亮显示前 100 行，然后使用 `setTimeout`（延时参数为 0）来安排（schedule）后 100 行的高亮显示，依此类推。
...
![图片](https://gitee.com/qdzhou/img-upload/raw/master/images/202202111754078.png)
...
3. - 出队（dequeue）并执行最早的微任务。
   - 当微任务队列非空时：

4. 执行渲染，如果有。

5. 如果宏任务队列为空，则休眠直到出现宏任务。
